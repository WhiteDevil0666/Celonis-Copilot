...
All modules merged into one file for Streamlit Cloud compatibility.
"""
...
.stButton>button {
    ...
    border:1px solid #2a3a5c !important; color:#c8d4e8 !important;
    ...
    line-height:1.5 !important; margin-bottom:3px !important;
    transition:all 0.18s !important;
}
.stButton>button:hover {
    background:rgba(47,110,245,0.2) !important;
    border-color:#2f6ef5 !important; color:#fff !important;
}
...
[data-testid="stTabs"] [data-baseweb="tab"] { color:#9ab0cc !important; }
...
/* forms */
[data-testid="stForm"] {
    background:#111627 !important; border:1px solid #2a3a5c !important;
...
.cel-title { font-size:18px; font-weight:700; color:#fff; margin:0; }
...
def init_rate_limiter():
    ...
    if "rl_tier" not in st.session_state:
        st.session_state.rl_tier = "free"

def check_rate_limit(user_id: str = "anon") -> tuple[bool, str]:
    init_rate_limiter()
...
def get_tavily_key() -> str:
    return (st.session_state.get("tavily_key_input", "")
            or st.secrets.get("TAVILY_API_KEY", ""))

def search_available() -> bool:
    return TAVILY_OK and bool(get_tavily_key())
...
def search_celonis(query: str, max_results: int = 4) -> list[dict]:
    ...
    try:
        ...
        _log("info", f"Web search '{query}' → {len(results)} results")
        return results
    except Exception as e:
        _log("error", f"Tavily error: {e}")
        return []
...
# ══════════════════════════════════════════════════════════════════
MODE_INSTRUCTIONS = {
    "beginner": "Use simple language, explain every term, use real-world analogies, be encouraging. Add 'Plain English' summaries.",
    "standard": "Use ### headings, code blocks, **bold** key terms, 💡 Pro Tip, ⚠️ Common Pitfall, numbered steps.",
    "expert":   "Skip basics. Deep architecture, performance, edge cases, advanced PQL patterns, trade-offs, enterprise considerations.",
    "pql_only": "Answer ONLY with PQL code. Show complete snippets, explain each function, show alternatives, include performance notes.",
}
...
- Celonis Studio (views, KPIs, action flows, apps)
- OLAP Views (creating, PQL aggregations, dimensions, measures)
- Process Explorer (variant analysis, conformance, happy path)
- Data Models (tables, foreign keys, activity tables, event logs)
- PQL — aggregations, CASE, SOURCE(), TARGET(), REMAP_TIMESTAMPS()
- Connectors (SAP ECC, S/4HANA, Salesforce, ServiceNow)
- ML Workbench, Permissions, Action Engine, KPI Trees

Always search docs.celonis.com for the most current information.
...
with left_col:
    ...
    groq_key = (
        st.text_input("Groq Key", type="password", placeholder="gsk_...",
                      label_visibility="collapsed", key="groq_key_input",
                      help="Free at console.groq.com")
        or st.secrets.get("GROQ_API_KEY", "")
    )
    st.text_input("Tavily Key (Web Search)", type="password", placeholder="tvly_... (optional)",
    ...
    with c2:
        if TIKTOKEN_OK:
            st.success("🧮 Tiktoken", icon="✅")
        else:
            st.info("🧮 Approx.", icon="ℹ️")
    ...
    with col_a:
        st.metric("Tokens", f"{stats['total']:,}")
    with col_b:
        st.metric("Cost", cost_estimate(stats["total"]))
...
with right_col:
    ...
    <div class="cel-header">
      <div class="cel-header-left">
        ...
        <div>
          ...
          <p class="cel-sub">{' · '.join(feat)} · {answer_mode.replace('_',' ').title()} mode</p>
        </div>
      </div>
    ...
    if not st.session_state.messages:
        ...
        <div class="welcome-box">
          ...
          <div class="wel-sub">
            I combine live Celonis documentation search and expert AI to give you
            accurate, enhanced answers with step-by-step guides, PQL examples, and best practices.<br/><br/>
    ...
    if prompt:
        # Groq key check
        if not groq_key:
            st.error("⚠️ Enter your **Groq API Key** in the left panel. Free key → [console.groq.com](https://console.groq.com)")
            st.stop()

        # Rate limit check
        allowed, rl_msg = check_rate_limit("user")
        if not allowed:
            st.warning(rl_msg)
            st.stop()
        ...
        with st.chat_message("assistant", avatar="⚡"):
            ...
            try:
                ...
                record_tokens(prompt_token_count, resp_tokens)
                latency = (time.time() - t0) * 1000
                _log("info", f"QUERY model={selected_model} tokens={prompt_token_count+resp_tokens} latency={latency:.0f}ms q={prompt[:60]!r}")
                ...
                badges += f'<span class="badge b-green">⏱ {latency/1000:.1f}s · {resp_tokens} tokens</span>'
                badges += '</div>'
                ...
                _log("error", "AuthenticationError")
                st.error("❌ **Invalid Groq API Key.** Please check the left panel.")
            except groq_errors.RateLimitError:
                _log("error", "Groq RateLimitError")
                ...
                _log("error", f"APIStatusError {e.status_code}: {e.message}")
